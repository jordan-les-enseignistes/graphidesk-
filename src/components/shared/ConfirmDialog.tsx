...
export function ConfirmDialog({
  ...
  return createPortal(
    <>
      ...
      <div
        className={cn(
          "fixed left-[50%] top-[50%] z-[101] w-full max-w-md translate-x-[-50%] translate-y-[-50%] rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 shadow-lg animate-slide-in"
        )}
        onClick={(e) => e.stopPropagation()}
      >
        <button
          className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-white dark:ring-offset-slate-800 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:text-slate-400"
          onClick={() => onOpenChange(false)}
        >
          <X className="h-4 w-4" />
        ...
        <div className="flex flex-col space-y-1.5 text-left">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="text-lg font-semibold leading-none tracking-tight dark:text-slate-100">{title}</h2>
